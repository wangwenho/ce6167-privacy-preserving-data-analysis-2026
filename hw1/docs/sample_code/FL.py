...
class FederatedLearning:
    ...
    def evaluate(self, model, test_loader, criterion=None):
        ...
        return Test_loss, acc, f1, all_preds, all_targets # return preds, targets for final_evaluation
...
def main():
    ...
    client_dataloaders = {
        client: {
            "train": DataLoader(datasets.ImageFolder(path["train"], transform=basic_transform), batch_size=batch_size, shuffle=True),
            "test": DataLoader(datasets.ImageFolder(path["test"], transform=basic_transform), batch_size=batch_size, shuffle=False)
        } for client, path in client_dataset_paths.items()
    }

    server_dataloaders = {
        "Server": {
            "test": DataLoader(datasets.ImageFolder(server_dataset_path["Server"]["test"], transform=basic_transform), batch_size=batch_size, shuffle=False)
        }
    }
    ...
    fl = FederatedLearning(client_dataloaders, server_dataloaders, epochs=epoch, batch_size=batch_size, lr=lr, k_folds=k_folds)
