...
# noise=0 在 log scale 上無法顯示，用一個小值取代，標籤手動寫 "0"
EPS = 1e-4
...
# ============================================================
# Figure 1: 生成品質指標（ROUGE-1, BLEU-4, Exact Match）
# ============================================================
fig, ax1 = plt.subplots(figsize=(9, 5.5))

# 主 Y 軸 (0~1)
ax1.plot(x, df['rouge1'], 'o-',  color='#E74C3C', linewidth=2, markersize=6, label='ROUGE-1')
ax1.plot(x, df['bleu4'],  's-',  color='#3498DB', linewidth=2, markersize=6, label='BLEU-4')
ax1.plot(x, df['exact_match'], '^-', color='#2ECC71', linewidth=2, markersize=6, label='Exact Match')
ax1.axhline(y=0, color='gray', linewidth=0.5, linestyle='--')

# 右側 Y 軸 (Edit Distance)
ax2 = ax1.twinx()
ax2.plot(x, df['edit_mean'], 'D--', color='#9B59B6', linewidth=2, markersize=5,
         alpha=0.7, label='Edit Distance (mean)')
...
ax1.text(0.022, 0.92, 'Threshold\nRegion', fontsize=10, color='orange',
...
ax1.set_title('GEIA Attack Quality vs. Gaussian Noise on Embeddings', fontsize=14, fontweight='bold')

plt.tight_layout()
plt.savefig('assets/fig1_generation_quality.png', dpi=150)
plt.close()
print('[OK] fig1_generation_quality.png')


# ============================================================
# Figure 2: 語意相似度（Embedding Similarity）
# ============================================================
...
ax.set_title('Semantic Similarity (sentence-t5-xxl) vs. Noise', fontsize=14, fontweight='bold')

plt.tight_layout()
plt.savefig('assets/fig2_embedding_similarity.png', dpi=150)
plt.close()
print('[OK] fig2_embedding_similarity.png')


print('\nDone! Figures saved to assets/')